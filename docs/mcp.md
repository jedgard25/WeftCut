...
- `split_layer { layer_id, at_t_us, escape_link? }` → `{ left, right }` — link-aware: a linked pair is cut at ONE frame instant (the video's precision dominates), so picture and sound never drift apart; unlinked Audio keeps sample precision.
